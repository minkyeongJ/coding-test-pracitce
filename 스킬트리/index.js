function solution(skill, skill_trees) {
  ...
  const skillTreesElementArr = skill_trees.map((skillTreesElement) =>
    Array.from(skillTreesElement)
  );
    ...
    skillArr.forEach((skillElement, j) => {
      for (let number = 0; number < skillTreesElement.length; number++) {
        if (skillTreesElement[number] === skillElement) {
          ...
          skillTreesElement.length === number + 1 &&
          checkArr[i].length === j
        ) {
  ...
  checkArr.forEach((checkArrElement) => {
    //배열의 길이가 1이고 값이 27이거나 skill의 길이가 1이면 answer++
    if (
      (checkArrElement.length === 1 && checkArrElement[0] === 27) ||
      skill.length === 1
    ) {
      answer++;
    }
